...
export function getRecoil(
  ...
  if (move.recoil) {
    ...
    case 2: case 3: case 4:
      ...
      break;
    default:
      recoil = notation === '%' ? 36 : 75;
      text = '50% crash damage';
    }
  } else if (move.struggleRecoil) {
    recoil = notation === '%' ? 18 : (375/1000);
...
function getHazards(gen: Generation, defender: Pokemon, defenderSide: Side) {
  ...
  if (defenderSide.isSR && !defender.hasAbility('Magic Guard', 'Mountaineer')) {
    ...
    damage += Math.floor((effectiveness * defender.maxHP()*1.5) / 8);
  ...
  if (!defender.hasType('Flying') &&
    ...
    if (defenderSide.spikes === 1) {
      damage += Math.floor(defender.maxHP()*1.5 / 8);
      if (gen.num === 2) {
        texts.push('Spikes');
      } else {
      ...
      damage += Math.floor(defender.maxHP()*1.5 / 6);
      texts.push('2 layers of Spikes');
    } else if (defenderSide.spikes === 3) {
      damage += Math.floor(defender.maxHP()*1.5 / 4);
...
function getEndOfTurn(
  ...
  if (field.hasWeather('Sun', 'Harsh Sunshine')) {
    ...
    if (defender.hasAbility('Dry Skin')) {
      ...
      damage += Math.floor(defender.maxHP()*1.5 / 16);
    ...
    if (
      ...
      damage -= Math.floor(defender.maxHP()*1.5 / (gen.num === 2 ? 8 : 16));
  ...
  if (field.attackerSide.isSeeded && !attacker.hasAbility('Magic Guard')) {
    let recovery = Math.floor(attacker.maxHP()*1.5 / (gen.num >= 2 ? 8 : 16));
  ...
  if (defender.hasStatus('psn')) {
    if (defender.hasAbility('Poison Heal')) {
      ...
      damage -= Math.floor(defender.maxHP()*1.5 / (gen.num === 1 ? 16 : 8));
    ...
    if (defender.hasAbility('Heatproof')) {
      damage -= Math.floor(defender.maxHP()*1.5 / (gen.num > 6 ? 32 : 16));
      texts.push('reduced burn damage');
    } else if (!defender.hasAbility('Magic Guard')) {
      damage -= Math.floor(defender.maxHP()*1.5 / (gen.num === 1 || gen.num > 6 ? 16 : 8));
  ...
  if (!defender.hasAbility('Magic Guard') && TRAPPING.includes(move.name) && gen.num > 1) {
    if (attacker.hasItem('Binding Band')) {
      damage -= gen.num > 5 ? Math.floor(defender.maxHP()*1.5 / 6) : Math.floor(defender.maxHP()*1.5 / 8);
      texts.push('trapping damage');
    } else {
      damage -= gen.num > 5 ? Math.floor(defender.maxHP()*1.5 / 8) : Math.floor(defender.maxHP()*1.5 / 16);
      texts.push('trapping damage');
    }
  }
  if (field.defenderSide.isSaltCured && !defender.hasAbility('Magic Guard')) {
    const isWaterOrSteel = defender.hasType('Water', 'Steel');
    damage -= Math.floor(defender.maxHP()*1.5 / (isWaterOrSteel ? 4 : 8));
...
function computeKOChance(
  ...
  let toxicDamage = 0;
  if (toxicCounter > 0) {
    toxicDamage = Math.floor((toxicCounter * maxHP*1.5) / 16);
    toxicCounter++;
  }
...
function predictTotal(
  ...
  if (toxicCounter > 0) {
    for (let i = 0; i < hits - 1; i++) {
      toxicDamage += Math.floor(((toxicCounter + i) * maxHP*1.5) / 16);
    }
    lastTurnEot -= Math.floor(((toxicCounter + (hits - 1)) * maxHP*1.5) / 16);
  }
  let total = 0;
